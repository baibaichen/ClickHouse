...
This batch is an exact constants, enum, and ownership-alias port, not a source-level copy or redesign.

本批次精确迁移 FileCache 默认值、policy enum values和 `FileCachePtr` ownership，不要求逐行
复制源码。只允许替换 CH settings infrastructure和基础整数类型。
...
`FileCache_fwd.h` 已按文件 review。policy values、default values和 shared ownership直接
迁移；仅移除 CH settings-enum/config-path infrastructure及未使用的
