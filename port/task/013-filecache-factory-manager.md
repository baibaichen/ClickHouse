...
| `OpenedFileCache` (CH `src/IO/OpenedFileCache.h`) | not ported by any earlier task; this task migrates it fresh as a Manager-owned member (`openedFileCache_`), created as its own FileCache-side header (`velox/ch/Interpreters/FileCache/OpenedFileCache.h`) | behavioral source: `src/IO/OpenedFileCache.h`. Preserve CH's exact structure: a fixed 1024-bucket vector of bucket-local maps keyed by `(path, flags)` (CH: `std::map` via `MapWithMemoryTracking`), each bucket guarded by its own `std::mutex` and holding `std::weak_ptr<OpenedFile>` values. Allocate both the bucket vector and every per-bucket map with Velox `memory::StlAllocator<T>` (`velox/common/memory/MemoryPool.h`), constructed from the Manager's injected `memory::MemoryPool &`, so every container allocation is charged to that pool; a plain, untracked container (no `MemoryPool`-backed allocator) is forbidden. Map `ProfileEvents::OpenedFileCacheHits`/`OpenedFileCacheMisses`/`OpenedFileCacheMicroseconds` to the existing Task-003 no-op `ProfileEvents` shim: these three names are already added to `ProfileEvents.h` by Task 003 (34-name B1 list); Task 013 only consumes them and must not edit `ProfileEvents.h`. The bucket-selection hash (`CityHash_v1_0_2::CityHash64`) has no CH-visible consumer contract — no caller depends on which physical bucket an entry lands in, only on `get`/`remove` correctness and weak-pointer sharing — so it is an approved, user-confirmed internal-only infra substitution: use `std::hash<std::string>{}(path)` for the 1024-bucket selection. This substitution applies only to bucket selection; it does not extend to any other CH primitive. If any other CH primitive surfaces while porting this file (e.g. a lifecycle hook not covered here), stop at the gate instead of improvising. |
