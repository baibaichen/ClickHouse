...
这个类不需要完整复刻 ClickHouse 整套 IO 框架。只保留
`CachedOnDiskReadBufferFromFile` 路径实际用到的 `ReadBufferFromFileBase` 能力：
...
```text
REMOTE_FS_READ_AND_PUT_IN_CACHE:
    ReadBufferFromVeloxReadFile 挂到 FileSegment::DownloadState
    foreground/background downloader 可复用

REMOTE_FS_READ_BYPASS_CACHE:
    ReadBufferFromVeloxReadFile 是当前 FileCacheInputStream 的局部状态
    不挂到 FileSegment
```
